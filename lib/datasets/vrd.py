...
class vrd(imdb):
    ...
    def _do_python_eval(self, output_dir, pickle1=True, eval_attributes = False):
        ...
        for i, cls in enumerate(classes):
            ...
            if npos > 1 and type(scores)==list :
                f = np.nan_to_num((prec * rec) / np.maximum((prec + rec), np.finfo(np.float64).eps))
                print(scores[np.argmax(f)])
        ...
        avg_thresh = np.mean(thresh[thresh!=0]) if thresh.sum() > 0 else 0
